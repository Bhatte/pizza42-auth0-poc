...
The SPA is served with its own headers from [../web/vercel.json](../web/vercel.json): a Content Security Policy, HSTS, `nosniff`, `frame-ancestors 'none'`, a referrer policy and a Permissions-Policy denying device APIs the storefront never uses. Session restoration uses the rotating refresh token persisted by `auth0-spa-js`; this configuration does not create the SDK's memory-cache Web Worker and does not enable its iframe fallback. The policy therefore grants neither `worker-src blob:` nor a tenant `frame-src`. The Auth0 domain remains in `connect-src` for token exchange, and the API origin is named there for application calls. Both are literals because Vercel does not expand environment variables in header values; changing either deployment target means changing the policy in the same commit.
