...
- No custom email provider is configured, so verification and password-reset mail is Auth0's stock template on its built-in development service: rate-limited, Auth0-branded, and sent from an unmonitored address whose closing line tells the customer to reply to it. Changing a single word of that requires connecting an email provider first — a tenant prerequisite, not a code change.
